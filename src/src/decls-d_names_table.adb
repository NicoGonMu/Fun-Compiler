...
package body decls.d_names_table is
   ...
   procedure put (nt: in out names_table; name: in string; id: out name_id) is
      ...
      p: name_id;
   begin
      i := hash(name) mod b; p := dt(i);
      while p /= null_id and then not equal(name, nt, p) loop
         p := idt(p).psh;
      end loop;
      if p = null_id then
         ...
         save_id(name, ct, cn);
         nid := nid + 1; idt(nid) := (dt(i), cn);
         dt(i) := nid; p := nid;
      end if;
      id := p;
   end put;

   procedure put (nt: in out names_table; name: in string; id: out str_id) is
      strt: str_table renames nt.strt;
      ct:   chr_table renames nt.ct;
      strs: str_id    renames nt.strs;
      cn:   natural   renames nt.cn;
   ...
   begin
      ...
      ip := idt(id - 1).ptc + 1; fp := idt(id).ptc;
      return ct(ip..fp);
   end consult;

   function consult (nt: in names_table; id: in str_id) return string is
      strt: str_table renames nt.strt;
      ct:   chr_table renames nt.ct;
