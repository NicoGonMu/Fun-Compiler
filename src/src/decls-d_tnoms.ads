...
package decls.d_names_table is
   ...
   procedure put     (tn: in out names_table; name: in string; id: out str_id);
   function  consult (tn: in names_table; id: in str_id) return string;

   out_of_capacity: exception;
   bad_use: exception;

private
   max_ch: constant natural := max_id*long_m_id + max_str*long_m_str;
   ...
   type names_table is
      record
         dt:  disp_table;	--Dispersion Tabñe
         idt: id_table;		--name_ids Tabke
